...
class _OccupationDetailViewPageState extends State<OccupationDetailViewPage> {
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    _buildInfoBox(
                      'Flat/Building No:',
                      subtitle: business.flatNo?.toString() ?? "N/A",
                    ),
                    const SizedBox(height: 20),

                    _buildInfoBox(
                      'Address:',
                      subtitle: business.address?.toString() ?? "N/A",
                    ),
                    const SizedBox(height: 20),

                    _buildInfoBox(
                      'Area/Locality:',
                      subtitle: business.areaName?.toString() ?? "N/A",
                    ),
                    const SizedBox(height: 20),

                    _buildInfoBox(
                      'Country:',
                      subtitle: business.countryName?.toString() ?? "N/A",
                    ),
                    const SizedBox(height: 20),

                    _buildInfoBox(
                      'State:',
                      subtitle: business.stateName?.toString() ?? "N/A",
                    ),
                    const SizedBox(height: 20),

                    _buildInfoBox(
                      'City:',
                      subtitle: business.cityName?.toString() ?? "N/A",
                    ),
                    const SizedBox(height: 20),

                    _buildInfoBox(
                      'Pincode:',
                      subtitle: business.pincode?.toString() ?? "N/A",
                    ),
                    const SizedBox(height: 20),
  ...
  void _showDeleteConfirmationDialog(BuildContext context) {
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: const Text("Delete Business Details"),
          content: const Text("Are you sure you want to delete these business details? This action cannot be undone."),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(context),
  ...
  Future<void> _deleteBusinessDetails() async {
    try {
      // Implement your delete business details API call here
      // await controller.deleteBusinessDetails();
      ...
      Get.snackbar(
        "Error",
        "Failed to delete business details: $e",
        backgroundColor: Colors.red,
        colorText: Colors.white,
      );
    }
