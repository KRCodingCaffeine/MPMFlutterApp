...
class ApplicantDetailYourself extends StatefulWidget {
  const ApplicantDetailYourself({super.key});

  @override
  State<ApplicantDetailYourself> createState() =>
      _ApplicantDetailYourselfState();
}
...
class _ApplicantDetailYourselfState extends State<ApplicantDetailYourself> {
    ...
    try {
      ...
      if (response.status == true && response.data != null) {
        setState(() {
          ...
          // 🔥 Prefill DOB and calculate age
          if (_applicationData?.dateOfBirth != null &&
              _applicationData!.dateOfBirth!.isNotEmpty) {
            final parsedDob = DateTime.tryParse(_applicationData!.dateOfBirth!);

            if (parsedDob != null) {
              dobCtrl.text = _formatDobForUi(_applicationData!.dateOfBirth!);

              _calculateAgeFromDob(parsedDob);
            }
          }
        });
      } else {
  ...
  void _prefillFromLoggedInUser() {
    final user = controller.getUserData.value;
    ...
    if (user.dob != null && user.dob!.isNotEmpty) {
      dobCtrl.text = _formatDobForUi(user.dob!);

      try {
        final parsed = DateTime.parse(user.dob!);
        _calculateAgeFromDob(parsed);
      } catch (_) {}
    }
    ...
    motherNameCtrl.text = user.motherName ?? '';
  ...
  Future<void> _submitApplicantAndFather() async {
    ...
    try {
      ...
      if (createResponse.status != true) {
        throw Exception(
            createResponse.message ?? "Failed to create application");
      }
  ...
  void _openEditApplicant() {
    ...
    if (data.dateOfBirth != null && data.dateOfBirth!.isNotEmpty) {
      final parsed = DateTime.tryParse(data.dateOfBirth!);
      if (parsed != null) {
        _calculateAgeFromDob(parsed);
      }
    }
