...
class NewMemberController extends GetxController  {
  ...
  setRelationShipType(List<RelationData> _value) => relationShipTypeList.value =_value;
  ...
  void getOccupationData() {
   setRxRequestOccuption(Status.LOADING);
    api.userOccupationDataApi().then((_value) {
      ...
      occuptionList.add(OccupationData(id: "other", occupation: 'Other', status: '1', createdAt: null, updatedAt: null));
    }).onError((error,strack) {
  ...
  void getOccupationSpectData(String occupation_profession_id){
    Map datas={
      "occupation_profession_id":occupation_profession_id
    };
 ...
 void userRegister(var LM_code,BuildContext context) async{
   ...
   var full_address=stateController.value.text+""+cityController.value.text+""+zoneController.value.text+""+areaController.value.text;
   ...
   //var token=await SessionManager.getToken();
    var     token= "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIyMSIsImlhdCI6MTczNTQ2NjMyOCwiZXhwIjoxNzM1NDY5OTI4fQ.YVCh_-zc1Cap8ERgkY9yRVduxalEFEqUPT8RRs-NRcg";
